...
The working project root for Activities 1-3 is always `my-mcp-app`. The files in `examples/` are reference snippets you can copy from, not the active project root.
...
color? Please specify..."              MCP App returns:
                                       ┌──────────────────┐
User replies: "The living room         │  Pick your bulb  │
bulb, warm white"                      │  ○ Living Room   │
                                       │  ○ Bedroom       │
AI returns: "OK, set to warm           │  Color: [████]   │
white."                                │  [Submit]        │
                                       └──────────────────┘
                                       User clicks Submit
...
- If participants get stuck on the Vite config, copy the structure from `examples/example-vite-config.ts` into `my-mcp-app/vite.config.ts`.
...
| Vite build fails                         | Missing `vite.config.ts` or wrong `build.rollupOptions`   | Copy the example Vite config from `examples/` into `my-mcp-app/vite.config.ts` and confirm the entry point path is correct  |
