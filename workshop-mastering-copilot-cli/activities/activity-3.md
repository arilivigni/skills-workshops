...
### Step A5 – Test the skill

Start the orchestrator and ask it to summarize recent commits:

```text
Summarize the last 10 commits in this repository.
```
...
### Step B2 – Create the Fleet manifest
...
### Step B3 – Create the output directory
...
Both agents start in parallel. When both complete, check the output files:
...
| Same task, want a quality check | Second-opinion model switch (`/model`) |
| Long multi-phase task with distinct roles | Orchestrated agents |
| Independent tasks that can run at the same time | Fleet |
| Reusable capability with helper code | Agent skill |
...
- [ ] The skill is attached to an agent config
...
|---|---|
| Skill not loading | Confirm the path is `.github/skills/git-summary/SKILL.md` and the `skills` list in the agent config matches exactly |
...
| One Fleet agent blocking the other | The agents may be sharing a resource — move each agent to an independent output file |
